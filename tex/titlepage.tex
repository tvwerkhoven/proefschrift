% !TEX TS-program = xelatex
% !TEX encoding = UTF-8 Unicode
%
%% Copyright 2010--2014 Tim van Werkhoven (t.i.m.vanwerkhoven@gmail.com)
%% 
%% This work is licensed under the Creative Commons
%% Attribution-Noncommercial-Share Alike 3.0 Unported License. To view a copy
%% of this license, visit http://creativecommons.org/licenses/by-nc-sa/3.0/ or 
%% send a letter to Creative Commons, 171 Second Street, Suite 300, San 
%% Francisco, California, 94105, USA.

\frontmatter
\pagestyle{titlestyle}
%%% disable page numbering
\hypersetup{pageanchor=false}
%%% Set small, symmetric 2cm margins
\setlrmarginsandblock{2cm}{2cm}{*}
\setulmarginsandblock{2cm}{2cm}{*}
\checkandfixthelayout
% Magic happens here:
% http://tex.stackexchange.com/a/110230
\makeatletter
\ch@ngetext
\makeatletter
\clearpage


%%%%%%%%%%%%%%%%%
%%% COVERPAGE %%%
%%%%%%%%%%%%%%%%%

%%% First page, cover image, do not fit paper to image
%\includepdf[fitpaper=false]{\imgpath \thesiscoverimg}
%\cleardoublepage

%%%%%%%%%%%%%%%%%%%%%%%%%
%%% GENERAL TITLEPAGE %%%
%%%%%%%%%%%%%%%%%%%%%%%%%

\vspace*{\fill}
\begin{center}
{
  {\Huge \thesistitle }\\[0.5cm]
  {\huge ~\thesissubtitle~ }\\
}
\end{center}
\vspace*{\fill}
\vspace*{\fill}
\clearpage

\vspace*{\fill}
\ccbyncsa\ \thesisyear\ \thesisauthor\\
Some rights reserved -- \url{creativecommons.org/licenses/by-nc-sa/3.0/}\\

% Hack from http://www.tex.ac.uk/cgi-bin/texfaq2html?label=topgraph
\ifdefined\thesisonline
\thesisonline~\\
\fi

\ifdefined\thesisisbna
\textsc{ISBN-10}: \href{http://www.google.com/?q=\thesisisbna}{\thesisisbna}\\
\fi
\textsc{ISBN-13}: \href{http://www.google.com/?q=\thesisisbnb}{\thesisisbnb}\\

\ifdefined\thesisprinter
Printed by \thesisprinter
\fi

\thesisprintdetails\\

\ifdefined\thesiscoverimgdesc%
\emph{Cover:} \thesiscoverimgdesc\\
\fi
\clearpage

%%%%%%%%%%%%%%%%%%%%%%%%
%%% LEIDEN TITLEPAGE %%%
%%%%%%%%%%%%%%%%%%%%%%%%

% N.B. The last line needs to be manually justified because there is no center-justified environment.
% From http://onderzoek.leidenuniv.nl/phd/phd-kandidaten/proefschrift/titelblad.html
% Also from titels.tex by Alfred de Wijn.
\begin{center}
  {
    {\Huge \thesistitle }\\[1em]
    {\huge \thesissubtitle }\\[5em]
    {\Large Proefschrift}\\[5em]
    {\Large \parbox{0.55 \textwidth}{ter verkrijging van de graad van Doctor aan de Universiteit Leiden, op gezag van Rector Magnificus prof\@. mr\@. C.\,J.\,J.\,M\@. Stolker,
volgens besluit van het College voor Promoties
te verdedigen op donderdag \linebreak\centerline{26 juni 2014 klokke 10.00 uur}}}\vfill
	{\Large door}\\[3em]
	{\LARGE \thesisauthorfull}\\[3em]
	{\Large \thesisauthorborn}
%	{\parbox[c][][c]{0.65 \textwidth}{\centering\Large\thesisauthor\\ \thesisauthorborn}}
  }
\end{center}

\clearpage

%%% Promotor(s) / promotiecommissie -- centered style
%\begin{center}
%\large%
%\makebox[0pt][r]{Promotoren: }\makebox[0pt][l]{\parbox[t]{0.5\textwidth}{%
%\thesispromotores}}\\[1cm]
%\makebox[0pt][r]{Promotiecommissie: }\makebox[0pt][l]{\parbox[t]{0.5\textwidth}{%
%\thesiscomittee}}
%\end{center}

%%% Promotor(s) / promotiecommissie -- classical left-aligned style
{%
\ifdefined\thesiscomittee%
\large%
\makebox[0pt][l]{Promotoren:}\hspace{3.6cm}\parbox[t]{0.7\textwidth}{%
\thesispromotores}\\[1cm]
\makebox[0pt][l]{Promotiecommissie:}\hspace{3.6cm}\parbox[t]{0.7\textwidth}{%
\thesiscomittee}
\else
\large%
{Promotoren:}\quad\parbox[t]{0.7\textwidth}{%
\thesispromotores}\\[1cm]
\fi
}
\vfill

{\thesisfunding}

\clearpage

%%%%%%%%%%%%%%%%%%
%%% DEDICATION %%%
%%%%%%%%%%%%%%%%%%

\begin{center}
  \vspace*{0.25\textheight}
  
  \emph{\thesisproverb}\\[1em]
  \thesisproverbauthor
  
  \vspace*{0.5\textheight}

  \emph{\thesisdedication}
\end{center}

%%%%%%%%%%%
%%% TOC %%%
%%%%%%%%%%%

%% Restore page/margin geometry
\thesisgeometry
\checkandfixthelayout
\makeatletter
\ch@ngetext
\makeatletter
\cleardoublepage

%% Enable page numbering again
\hypersetup{pageanchor=true}

\setcounter{tocdepth}{1}
%% Perhaps we want sections linking back to the TOC http://tex.stackexchange.com/questions/45603/hyperlink-to-the-toc-page http://tex.stackexchange.com/questions/32495/linking-the-section-text-to-the-toc
\hypertarget{thesistoc}{}
\tableofcontents

%% Clear the remaining pages, reset style
\pagestyle{thesisstyle}
\mainmatter
\cleardoublepage
